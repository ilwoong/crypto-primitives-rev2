...
static const aes_variant VARIANTS[] = {
    {"aes", &aes128_block_cipher, &aes192_block_cipher, &aes256_block_cipher},
    {"aes-lut1", &aes128_lut1_block_cipher, &aes192_lut1_block_cipher, &aes256_lut1_block_cipher},
    {"aes-lut2", &aes128_lut2_block_cipher, &aes192_lut2_block_cipher, &aes256_lut2_block_cipher},
    {"aes-lut3", &aes128_lut3_block_cipher, &aes192_lut3_block_cipher, &aes256_lut3_block_cipher},
};
...
static const block_cipher *select_cipher(const aes_variant *v, size_t key_bytes)
{
    switch (key_bytes) {
    case 16:
        return v->cipher128;
...
static int run_vector(const aes_variant *v, test_mode mode, size_t count, const uint8_t *key, size_t key_len,
                      const uint8_t *plaintext, const uint8_t *ciphertext)
{
    ...
    uint8_t ctx_buf[sizeof(aes_ctx)];
    uint8_t result[BLOCK_SIZE];

    cipher->expand_key(ctx_buf, key);

    if (mode == MODE_ENCRYPT) {
        cipher->encrypt(ctx_buf, result, plaintext);
        ...
        cipher->decrypt(ctx_buf, result, ciphertext);
