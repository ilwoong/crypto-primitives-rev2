...
static const lea_variant VARIANTS[] = {
    {"lea", &lea128_block_cipher, &lea192_block_cipher, &lea256_block_cipher},
    {"lea-unrolled", &lea128_unrolled_block_cipher, &lea192_unrolled_block_cipher, &lea256_unrolled_block_cipher},
};
...
static const block_cipher *select_cipher(const lea_variant *v, size_t key_bytes)
{
    switch (key_bytes) {
    case 16:
        return v->cipher128;
...
static int run_vector(const lea_variant *v, test_mode mode, size_t count, const uint8_t *key, size_t key_len,
                      const uint8_t *plaintext, const uint8_t *ciphertext)
{
    ...
    uint8_t ctx_buf[sizeof(lea_ctx)];
...
int main(int argc, char *argv[])
{
    ...
    printf("LEA KAT (Known Answer Test)\n");
