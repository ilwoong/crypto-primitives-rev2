...
// CHAM-64/128 and CHAM-128/128 share a key length, so the block length is needed to disambiguate.
static const block_cipher *select_cipher(size_t key_bytes, size_t block_bytes)
{
    if (key_bytes == 16 && block_bytes == 8)
        return &cham64_128_block_cipher;
    if (key_bytes == 16 && block_bytes == 16)
        return &cham128_128_block_cipher;
    if (key_bytes == 32 && block_bytes == 16)
        return &cham128_256_block_cipher;
    return NULL;
}
...
int main(int argc, char *argv[])
{
    ...
    printf("CHAM KAT (Known Answer Test)\n");
